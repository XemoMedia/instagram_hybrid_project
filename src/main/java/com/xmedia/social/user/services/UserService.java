...
public class UserService {
	...
	public User saveGoogleUser(String name, String email, String loginType) {
		return userRepository.findByEmail(email).orElseGet(() -> {
			...
			User newUser = User.builder().username(name).email(email).password("") // no password for Google login
					.userType(UserType.valueOf(loginType.toUpperCase())).accountType(UserAccountType.PERSONAL)
